...
def test_zurlsamedb():
    ...
    # ZEO default sync is empty
    def presync(_): pass
    if isinstance(stor1, ZEOStorage):
        def presync(stor):
            # ZEO >= 5
            #   https://github.com/zopefoundation/ZEO/blob/423cb8/src/ZEO/ClientStorage.py#L224-L246
            if hasattr(stor, 'server_sync'):
                stor.server_sync = True
            # ping trick to force waiting for invalidations
            else:
                stor.sync = lambda: stor._server.lastTransaction()
