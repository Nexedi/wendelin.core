...
def test_zurlsamedb():
    ...
    # ZEO default sync is empty. Therefore we need to override the 'sync'
    # method of our storage. For ZEO 5 it would actually be sufficient to
    # set init parameter 'server_sync' to 'True':
    #   https://github.com/zopefoundation/ZEO/blob/423cb8/src/ZEO/ClientStorage.py#L224-L246
    # But because our storage is already initiated this doesn't help and we
    # need to manually override 'sync'.
    def patchsync(_): pass
    if isinstance(stor1, ZEOStorage):
        def patchsync(stor):
            # ZEO >= 5 specifies ping
            #   https://github.com/zopefoundation/ZEO/blob/423cb8/src/ZEO/ClientStorage.py#L472-L478

            # ZEO < 5: we need to assign ping method
            if not hasattr(stor, 'server_sync'):
                stor.ping = lambda: stor._server.lastTransaction()
